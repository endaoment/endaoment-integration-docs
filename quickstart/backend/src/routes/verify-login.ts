...
export const verifyLogin = async (req: Request, res: Response) => {
  ...
  if (!stateFromUrl || !code || typeof stateFromUrl !== 'string' || typeof code !== 'string') {
    res.status(400)
  ...
  const exportedVariables = JSON.parse(
    fs.readFileSync(
      path.join(import.meta.dirname, '../../login-states/', `${stateFromUrl}-exportedVariables.json`),
      'utf8',
    ),
  )
  ...
  formData.append('redirect_uri', EnvironmentVariables.ENDAOMENT_REDIRECT_URI())
  ...
  const tokenResponse = await fetch(staticUrl, {
    ...
    headers: {
      ...
      Authorization: `Basic ${Buffer.from(
        `${EnvironmentVariables.ENDAOMENT_CLIENT_ID()}:${EnvironmentVariables.ENDAOMENT_CLIENT_SECRET()}`,
      ).toString('base64')}`,
    },
    body: formData,
  })
